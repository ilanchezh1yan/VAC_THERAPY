...
uint16_t read_ADS1115(uint8_t pinselect) {
  esp_err_t err;
...
void offset_calibration (void) {
  ...
  uint32_t SumOfRawVlt = 0;
  uint8_t count = 0;

  while (count <= 100) {
    raw_voltage = read_ADS1115(0x60);
    if (raw_voltage) {
      SumOfRawVlt += (raw_voltage * 6114) / ADS1115_15_BIT_RESOLUTION;
      Serial.println(SumOfRawVlt);
      count++;
      delay(4);
    }
  }
  SensorRefVlt = SumOfRawVlt / count;
  Serial.println(SensorRefVlt);
}
